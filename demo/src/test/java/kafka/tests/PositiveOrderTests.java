...
@DisplayName("Позитивные сценарии обработки заказов")
public class PositiveOrderTests extends BaseTest {

    private static final Logger log = LoggerFactory.getLogger(PositiveOrderTests.class);
    private static final String INPUT_TOPIC = "orders.created";
    private static final String OUTPUT_TOPIC = "orders.processed";
    @ParameterizedTest(name = "{index}: {5}")
    @MethodSource("kafka.tests.data.TestDataFactory#providePositiveCases")
    void shouldAcceptValidOrders(String orderId, double amount, String currency, 
                                 OrderStatus expectedStatus, String description) {
        log.info(">>> Start Positive Test: {}", description);
        ...
        assertEquals(OrderStatus.ACCEPTED, result.getStatus(), 
                     "Status should be ACCEPTED for: " + description);
        assertNotNull(result.getReason(), "Reason should not be empty");
